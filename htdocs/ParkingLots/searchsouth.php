...
<?php
	...
	$sth3 = $handler->prepare("SELECT last_name1 FROM south");
	$sth3->execute();
	$studentLastNames1 = $sth3->fetchAll(PDO::FETCH_COLUMN, 0);

	$sth4 = $handler->prepare("SELECT first_name1 FROM south");
	$sth4->execute();
	$studentFirstNames1 = $sth4->fetchAll(PDO::FETCH_COLUMN, 0);

	$sth5 = $handler->prepare("SELECT last_name2 FROM south");
	$sth5->execute();
	$studentLastNames2 = $sth5->fetchAll(PDO::FETCH_COLUMN, 0);

	$sth6 = $handler->prepare("SELECT first_name2 FROM south");
	$sth6->execute();
	$studentFirstNames2 = $sth6->fetchAll(PDO::FETCH_COLUMN, 0);

	$sth7 = $handler->prepare("SELECT parking_spot FROM south");
	$sth7->execute();
