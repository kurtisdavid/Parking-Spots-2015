...
<div id="layout">
    ...
    <div id="main">
        <div class="header">
            <h1>North Lot Parking</h1>
            <h2>Add and search students.</h2>
        </div>
    </div>
</div>
...
<?php
	...
	$sth3 = $handler->prepare("SELECT last_name1 FROM north");
	$sth3->execute();
	$studentLastNames1 = $sth3->fetchAll(PDO::FETCH_COLUMN, 0);

	$sth4 = $handler->prepare("SELECT first_name1 FROM north");
	$sth4->execute();
	$studentFirstNames1 = $sth4->fetchAll(PDO::FETCH_COLUMN, 0);

	$sth5 = $handler->prepare("SELECT last_name2 FROM north");
	$sth5->execute();
	$studentLastNames2 = $sth5->fetchAll(PDO::FETCH_COLUMN, 0);

	$sth6 = $handler->prepare("SELECT first_name2 FROM north");
	$sth6->execute();
	$studentFirstNames2 = $sth6->fetchAll(PDO::FETCH_COLUMN, 0);

	$sth7 = $handler->prepare("SELECT parking_spot FROM north");
	$sth7->execute();
		...
		if ($student_ID == $studentIDs1[$i])
		{
			...
			if ($studentIDs2[$i]!=null)
			{
				...
				echo "<div class=\"content\">
						...
						<a href=northlot.php style=\"font-family:Raleway\"><button class=\"button-success pure-button\">Success!</button></a>
					  </div>";
			}
			else{
				echo "<div class=\"content\">
