...
if (strlen(str_replace(' ', '', $student_name))!=strlen($student_name) && $student_name&&$student_ID&&$student_grade&&!in_array($student_ID,$studentIDs1))
{
	...
	$sql = "INSERT INTO north ( parking_spot, last_name1, first_name1, studentID1, grade_level1 ) VALUES ( :parkingspot, :lastname, :firstname, :student_ID, :student_grade )";
	...
	echo "<div class=\"content\">
			<br><h1 style=\"color:black\"> $student_name's Parking Spot is: #$parkingspot! </h1>	
			<a href=northlot.php><button class=\"button-success pure-button\">Success!</button></a>
		  </div>";
...
#adds the two new students to the database
	...
	if ((strlen(str_replace(' ', '', $student_name1))!=strlen($student_name1)&&strlen(str_replace(' ', '', $student_name2))!=strlen($student_name2))&&($student_name1&&$student_ID1&&$student_grade1)&&($student_name2&&$student_ID2&&$student_grade2)&&((!in_array($student_ID2,$studentIDs2)&&!in_array($student_ID2,$studentIDs1))&&(!in_array($student_ID1,$studentIDs1)&&!in_array($student_ID1,$studentIDs2))))
	{
		...
		$parkingspot = $possible_spaces[floor(mt_rand(0,$possible_spaces_length-1))];
		$sql = "INSERT INTO north ( parking_spot, last_name1, first_name1, studentID1, grade_level1, last_name2, first_name2, studentID2, grade_level2 ) 
		...
		echo "<div class=\"content\">
				<br><h1 style=\"color:black\"> $student_name1 and $student_name2's Parking Spot is: #$parkingspot </h1>
			 	<a href=northlot.php><button class=\"button-success pure-button\">Success!</button></a>
			  </div>";
...

	{
                ...
                {
                     echo "<div class=\"content\"><h2 style=\"color:black\" >$student_name's ID is already assigned a spot. Please check the input data.</h2></div>";
                }
                elseif (in_array($student_ID2,$studentIDs2) || in_array($student_ID2, $studentIDs1))
                {
                     echo "<div class=\"content\"><h2 style=\"color:black\" >$student_name2's ID is already assigned a spot. Please check the input data.</h2></div>";
                }
                else
                {
                     echo "<div class=\"content\"><h2 style=\"color:black\" >Please enter in valid values.</h2></div>";
                }
	}
}
